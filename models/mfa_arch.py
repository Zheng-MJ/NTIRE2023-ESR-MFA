...
class CCM(nn.Module):
    def __init__(self, dim, growth_rate=2.8):
        super().__init__()
        hidden_dim = int(dim * growth_rate)
        self.Conv1 = nn.Conv2d(dim, 2*hidden_dim, 1, 1, 0)
        self.Conv2 = nn.Sequential(nn.Conv2d(2*hidden_dim,2* hidden_dim, (3,5), 1, padding='same',groups=2*hidden_dim),nn.Conv2d(2*hidden_dim,2* hidden_dim, (5,3), 1, padding='same',groups=2*hidden_dim))
        self.Act =nn.GELU()
...
class MFA(nn.Module):
    def __init__(self, dim=60, n_blocks=8, ffn_scale=2.0, upscaling_factor=4):
        super().__init__()
...
if __name__== '__main__':
    ...
    s = 4
    dummy_input = torch.randn((1,3,1280 // s,720 // s))
    model = MFA( upscaling_factor=s)
